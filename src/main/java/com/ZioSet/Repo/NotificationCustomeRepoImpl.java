...
public class NotificationCustomeRepoImpl  implements NotificationCustomeRepo {
	...
	public List<Notification> getNotificationByLimit(int page_no, int item_per_page) {
		...
		try{
			long result = 0 ;
			Query q = null ;
			 result = (long) entityManager.createQuery("SELECT count(a) FROM Notification a").getSingleResult();
			 q = entityManager.createQuery("from Notification a ", Notification.class);
	...
	public List<Notification> getNotificationByLimitAndSearch(String searchText, int pageNo, int perPage) {
		...
		try{
			...
			long result = (long) entityManager
					.createQuery(
							"SELECT count(a) FROM Notification a where  a.messsge LIKE :searchText ")
					.setParameter("searchText", "%" + searchText + "%").getSingleResult();
			 q = entityManager.createQuery(
					"from Notification a where a.messsge LIKE :searchText ",
					Notification.class);
	...
	public int getUnseenNotificationCount() {
		// TODO Auto-generated method stub
		Query q = null ;
		long result = (long) entityManager.createQuery("SELECT count(a) FROM Notification a where a.view_bit=1").getSingleResult();
		 q = entityManager.createQuery("from Notification a ", Notification.class);
