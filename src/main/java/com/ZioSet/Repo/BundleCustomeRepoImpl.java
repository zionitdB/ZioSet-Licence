...
public class BundleCustomeRepoImpl implements BundleCustomeRepo {
	@PersistenceContext
	...
	public int getBundleCountbySearch(String searchText) {
		...
		long result = (long) entityManager
				.createQuery(
						"SELECT count(b) FROM Bundle b where  b.bundleName LIKE :searchText")
	...
	public int getBundleTotalCount() {
		// TODO Auto-generated method stub
		long result = (long) entityManager.createQuery("SELECT count(b) FROM Bundle b").getSingleResult();
		int total_count = (int) result;
		return total_count;
	}
	...
	public List<Bundle> getBundleByLimitAndSearch(String searchText, int pageNo, int perPage) {
		...
		try {
			...
			Query q = entityManager.createQuery(
					"FROM Bundle b where  b.bundleName LIKE :searchText ",
					Bundle.class);
			...
			List<Bundle> list = q.getResultList();
			return list;
		} finally {
	...
	public List<Bundle> getBundlePagination(int page_no, int item_per_page) {
		...
		long result = (long) entityManager.createQuery("SELECT count(b) FROM Bundle b").getSingleResult();
		int total_count = (int) result;
		Query q = entityManager.createQuery("from Bundle b ", Bundle.class);
