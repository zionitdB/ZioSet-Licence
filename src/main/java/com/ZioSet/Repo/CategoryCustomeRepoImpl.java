...
public class CategoryCustomeRepoImpl implements CategoryCustomeRepo {
	...
	public List<Category> getCategoryPagination(int page_no, int item_per_page) {
		...
		try{
			long result = (long) entityManager.createQuery("SELECT count(c) FROM Category c").getSingleResult();
			int total_count = (int) result;
			Query q = entityManager.createQuery("from Category c ", Category.class);
	...
	public List<Category> getCategoryByLimitAndSearch(String searchText, int pageNo, int perPage) {
		...
		try {
			...
			Query q = entityManager.createQuery(
					"FROM Category c where  c.categoryName LIKE :searchText ",
					Category.class);
