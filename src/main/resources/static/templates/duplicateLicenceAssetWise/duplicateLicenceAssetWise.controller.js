(function() {
	...
	angular.module('myApp.duplicateLicenceAssetWise').controller('DuplicateLicenceAssetWiseController', DuplicateLicenceAssetWiseController);

	DuplicateLicenceAssetWiseController.$inject = [ '$state', '$uibModal','genericFactory', '$log','$scope', 'toastr' , 'localStorageService', 'ApiEndpoint','$window','$rootScope','$http','$filter','$stateParams'];
	
	/* @ngInject */
	function DuplicateLicenceAssetWiseController($state,$uibModal, genericFactory, $log, $scope, toastr, localStorageService, ApiEndpoint,$window,$rootScope,$http,$filter,$stateParams) {
		...
		function getAssetBySerialNo(serialNo){
			...
			genericFactory.getAll(msg,url).then(function(response) {
				...
				var 	url=licenceUrl+"/getAllLicenceByDuplicateProductByAssetId?assetId="+vm.asset.id;
				genericFactory.getAll(msg,url).then(function(response) {
