(function() {
	...
	angular.module('myApp.bundleWiseDetials').controller('BundleWiseDetialsController', BundleWiseDetialsController);

	BundleWiseDetialsController.$inject = [ '$state', '$uibModal','genericFactory', '$log','$scope', 'toastr' , 'localStorageService', 'ApiEndpoint','$window','$rootScope','$http','$filter'];
	
	/* @ngInject */
	function BundleWiseDetialsController($state,$uibModal, genericFactory, $log, $scope, toastr, localStorageService, ApiEndpoint,$window,$rootScope,$http,$filter) {
		...
		function loadBundles(){
			var msg="";
			var 	url=bundleUrl+"/getAllBundles";
			genericFactory.getAll(msg,url).then(function(response) {
				vm.bundles = response.data;
				console.log("types: "+JSON.stringify(vm.bundles))
