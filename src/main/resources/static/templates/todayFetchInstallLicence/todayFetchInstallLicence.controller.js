(function() {
	...
	angular.module('myApp.todayFetchInstallLicence').controller('TodayFetchInstallLicenceController', TodayFetchInstallLicenceController);

	TodayFetchInstallLicenceController.$inject = [ '$state', '$uibModal','genericFactory', '$log','$scope', 'toastr' , 'localStorageService', 'ApiEndpoint','$window','$rootScope','$http','$filter','$stateParams'];
	
	/* @ngInject */
	function TodayFetchInstallLicenceController($state,$uibModal, genericFactory, $log, $scope, toastr, localStorageService, ApiEndpoint,$window,$rootScope,$http,$filter,$stateParams) {
			...
			var msg=""
				...
				var url=""
					...
					var msg=""
						...
						url=licenceUrl+"/getTodaysFetchLicenceByLimit/"+vm.pageno+"/"+vm.perPage;
						urlCount=licenceUrl+"/getTodayFetchInstallLicenceCount"
