(function() {
	...
	angular.module('myApp.licenceDetialsBundle').controller('LicenceDetialsBundleController', LicenceDetialsBundleController);

	LicenceDetialsBundleController.$inject = [ '$state', '$uibModal','genericFactory', '$log','$scope', 'toastr' , 'localStorageService', 'ApiEndpoint','$window','$rootScope','$http','$filter'];
	
	/* @ngInject */
	function LicenceDetialsBundleController($state,$uibModal, genericFactory, $log, $scope, toastr, localStorageService, ApiEndpoint,$window,$rootScope,$http,$filter) {
		...
		$scope.newExcel= function(associate){
			console.log("FINALE: "+JSON.stringify(associate.applicationName))
		...
		function loadBundles(){
			var msg="";
			var 	url=bundleUrl+"/getBundleWiseCount";
		...
		function getApplicationByBundle(bundleName){
			var msg="";
			var 	url=bundleUrl+"/getApplicationCountByBundle?bundleName="+bundleName;
			console.log("URL "+url)
			genericFactory.getAll(msg,url).then(function(response) {
		...
		$scope.selectQR = function (index) {
			vm.bundles[index].check = !vm.bundles[index].check;
			
			 
			
			
			
			if(vm.bundles[index].check){
				$scope.selBundle=vm.bundles[index].bundleName
				getApplicationByBundle(vm.bundles[index].bundleName)

			}
			for(var i=0;i<=vm.bundles.length-1;i++){

				if(i==index){
				}else{
					vm.bundles[i].check=false
