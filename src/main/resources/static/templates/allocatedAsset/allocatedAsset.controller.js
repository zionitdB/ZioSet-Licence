(function() {
	...
	angular.module('myApp.allocatedAsset').controller('AllocatedAssetController', AllocatedAssetController);

	AllocatedAssetController.$inject = [ '$state', '$uibModal','genericFactory', '$log','$scope', 'toastr' , 'localStorageService', 'ApiEndpoint','$window','$rootScope','$http','$filter','$stateParams'];
	
	/* @ngInject */
	function AllocatedAssetController($state,$uibModal, genericFactory, $log, $scope, toastr, localStorageService, ApiEndpoint,$window,$rootScope,$http,$filter,$stateParams) {
		...
		function getAllAssets(){
			...
			if(sId==1){
			 ...
			 var 	url=assetUrl+"/getAllocatedAssetByBranch?dataReq="+dataReq
	...
	//***********************Pagination Start*****************************//
		...
		function loadAssets(){
			...
			*/
			url=assetUrl+"/getAllocatedAssetByBranch?dataReq="+dataReq
